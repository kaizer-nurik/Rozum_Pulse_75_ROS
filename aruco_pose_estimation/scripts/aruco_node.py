...
class ArucoNode(rclpy.node.Node):
    ...
    def image_callback(self, img_msg: Image):
        ...
        if len(markers.marker_ids) > 0:
            ...
            # Check if marker with ID 0 is detected and publish its transform
            for i, marker_id in enumerate(markers.marker_ids):
                self.get_logger().info(f"Marker ID: {marker_id}")
                if marker_id == 0:
                    # Create transform message
                    transform = TransformStamped()
                    transform.header = markers.header
                    transform.child_frame_id = self.camera_frame if self.camera_frame != "" else self.info_msg.header.frame_id
                    transform.child_frame_id = "camera_link"
                    transform.parent_frame_id = f"aruco_marker_{marker_id}"
                    
                    # Set translation
                    transform.transform.translation.x = markers.poses[i].position.x
                    transform.transform.translation.y = markers.poses[i].position.y
                    transform.transform.translation.z = markers.poses[i].position.z
                    
                    # Set rotation
                    transform.transform.rotation = markers.poses[i].orientation
                    
                    # Publish the transform
                    self.tf_broadcaster.sendTransform(transform)
                    self.get_logger().info(f"Transform published for marker {marker_id}")
                    # break
        ...
        if len(markers.marker_ids) > 0:
            ...
            for i, marker_id in enumerate(markers.marker_ids):
                ...
                if marker_id == 0:
                    ...
                    transform.header.frame_id = f"camera_link"
                    # transform.child_frame_id = self.camera_frame if self.camera_frame != "" else self.info_msg.header.frame_id
                    transform.child_frame_id = "aruco_marker_{marker_id}"
                    # transform.parent_frame_id = f"aruco_marker_{marker_id}"
                    
                    # Set translation
                    transform.transform.translation.x = pose_array.poses[i].position.z
                    transform.transform.translation.y = pose_array.poses[i].position.y
                    transform.transform.translation.z = pose_array.poses[i].position.x
                    # self.get_logger().info(f"Position: {pose_array.poses[i].position}"
                    transform.transform.rotation.x = pose_array.poses[i].orientation.x
                    transform.transform.rotation.y = pose_array.poses[i].orientation.y
                    transform.transform.rotation.z = pose_array.poses[i].orientation.z
                    transform.transform.rotation.w = pose_array.poses[i].orientation.w
                    # Set rotation
                    # Invert the orientation quaternion
                    # import tf_transformations, math
                    
                    # orientation = pose_array.poses[i].orientation
                    # # Convert quaternion to rotation matrix
                    # current_quat = [orientation.x, -orientation.y, -orientation.z, -orientation.w]
                    
                    # # Create 90-degree rotation around Y axis
                    # y_rotation_quat = tf_transformations.quaternion_from_euler(0, -math.pi/2, 0)
                    
                    # # Combine the rotations
                    # rotated_quat = tf_transformations.quaternion_multiply(current_quat, y_rotation_quat)
